...
class OpenCLWorkspace : public DeviceAPI {
 public:
  ...
  virtual bool IsOpenCLDevice(TVMContext device) {
    return device.device_type == kDLOpenCL;
  }
  // get the queue of the context
  cl_command_queue GetQueue(TVMContext device) {
    CHECK(IsOpenCLDevice(device));
    this->Init();
    CHECK(device.device_id >= 0  && static_cast<size_t>(device.device_id) < queues.size())
        << "Invalid OpenCL device_id=" << device.device_id;
