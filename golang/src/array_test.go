...
func TestArraydevice(t *testing.T) {
    ...
    device := arr.Getdevice()
    if device.DeviceType != KDLCPU {
        t.Errorf("device DeviceType expected: %v Got :%v\n", KDLCPU, device.DeviceType)
        return
    }
    if device.DeviceID != 0 {
        t.Errorf("device DeviceID expected: %v Got :%v\n", KDLCPU, device.DeviceID)
        return
    }
