...
def enabled_device_list():
    device_list = [('cpu', tvm.cpu(0)),
                ('gpu', tvm.gpu(0)),
                ('cl', tvm.opencl(0)),
                ('metal', tvm.metal(0)),
                ('rocm', tvm.rocm(0)),
                ('vulkan', tvm.vulkan(0)),
                ('vpi', tvm.vpi(0))]
    for k, v  in device_list:
        assert tvm.context(k, 0) == v
    device_list = [x[1] for x in device_list if x[1].exist]
    return device_list

ENABLED_device_LIST = enabled_device_list()
print("Testing using contexts:", ENABLED_device_LIST)


def test_nd_create():
    for device in ENABLED_device_LIST:
        for dtype in ["uint8", "int8", "uint16", "int16", "uint32", "int32",
                      "float32"]:
            x = np.random.randint(0, 10, size=(3, 4))
            x = np.array(x, dtype=dtype)
            y = tvm.nd.array(x, device=device)
            z = y.copyto(device)
