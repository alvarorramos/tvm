...
def test_save_load_out():
    ...
    def _run(env, remote):
        ...
        x_nd = tvm.nd.array(x_np, device)
        y_nd = tvm.nd.empty(y_np.shape, device=device, dtype=y_np.dtype)
...
def test_gemm():
    ...
    def _run(env, remote):
        ...
        def verify(s, name=None):
            ...
            x_nd = tvm.nd.array(x_np, device)
            w_nd = tvm.nd.array(w_np, device)
            y_nd = tvm.nd.array(y_np, device)
...
def test_alu():
    def _run(env, remote):
        def check_alu(tvm_op, np_op=None, use_imm=False, test_name=None):
            ...
            res_nd = tvm.nd.array(
                np.zeros((m, n, env.BATCH, env.BLOCK_OUT)).astype(res.dtype), device)
